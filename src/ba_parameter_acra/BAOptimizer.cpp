...
/**
 *	@file src/ba_parameter_acra/BAOptimizer.cpp
...
#include "ba_parameter_acra/BAOptimizer.h"
...
CBAOptimizer::CBAOptimizer(bool b_verbose, bool b_use_schur) // throw(srd::bad_alloc)
	:m_p_optimizer(new CBAOptimizerCore(b_verbose, b_use_schur)), m_n_last_camera_vertex(size_t(-1))
{}
...
{
	m_p_optimizer->r_System().r_Get_Vertex<CVertexCamSim3>(n_vertex_id, v_cam_state);
...
std::vector<double> Get_Eigenvalues(const cs *p_matrix, size_t n_eig_num, bool b_smallest = true);
...
public:
	CCalcRepErr(double &r_f_dest, bool &r_b_visible)
		:m_r_f_dest(r_f_dest), m_r_b_visible(r_b_visible)
	{}
...
void CBAOptimizer::Show_Stats() const
{
	...
	if(!edge_pool.b_Empty()) {
		for(size_t i = 0, n = edge_pool.n_Size(); i < n; ++ i) {
			...
			edge_pool.For_Each(i, i + 1, CCalcRepErr(f_reprojection_err, b_visible)); // need to derive the edge type
	...
	try {
		...
		cs *p_matrix_csc;
		{
			cs *p_matrix_upd = m_p_optimizer->r_Solver().r_Lambda().p_Convert_to_Sparse();
		...
		std::vector<double> lg_eig, sm_eig;
		sm_eig = Get_Eigenvalues(p_matrix_csc, 1);
		lg_eig = Get_Eigenvalues(p_matrix_csc, 1, false);
		cs_spfree(p_matrix_csc);

		double f_max_eig = lg_eig.front();
		double f_min_eig = sm_eig.front();
