...
/**
 *	@brief SE base edge implementation (specialization for binary edges)
 *
...
public:
	 ...
	 */
	enum {
		n_vertex_num = CTypelistLength<_TyVertices>::n_result, /**< @brief number of vertices */
...
private:
	...
	typename CVertexTraits<1>::_TyMatrixAlign m_t_HtSiH_vertex1; /**< @brief block of memory where Ht * inv(Sigma) * H matrix is stored (the vertex contribution) */
	...
	typename CVertexTraits<1>::_TyVectorAlign m_t_right_hand_vertex1; /**< @brief block of memory where right-hand side vector is stored (the vertex contribution) */
...
public:
	...
	double f_Max_VertexHessianDiagValue() const // todo surround with levenberg support ifdef
	{
		...
		Eigen::Map<typename CVertexTraits<1>::_TyMatrix,
			CUberBlockMatrix::map_Alignment> m_t_HtSiH_vertex1(m_p_HtSiH_vert[1]);
